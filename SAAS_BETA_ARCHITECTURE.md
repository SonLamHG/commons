...
4. ✅ **Hardening mạng** (ADR-6): CORS, rate-limit, security headers, SSRF guard, mã hoá webhook at-rest, trustProxy, bind host cấu hình được. (Proxy/TLS do nền tảng deploy lo — Bước 7.)
...
7. ✅ **Deploy** (portable artifacts): `Dockerfile` (node 24 + git + tsx), `docker-compose.yml`
   (app + Caddy auto-TLS + backup), `Caddyfile`, `scripts/backup.sh` (tested: git bundles +
   SQLite checkpoint), `DEPLOY.md`. Production `start` script thay `node --watch`. Chạy trên VPS
   bất kỳ hoặc Fly, single-node (ADR-1).
...
- ~~**Nền tảng deploy cụ thể** (Fly vs VPS)~~: ✅ chọn artifacts portable (Docker Compose +
  Caddy) chạy trên VPS bất kỳ, kèm ghi chú nâng cấp Fly trong `DEPLOY.md`.
